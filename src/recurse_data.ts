...
const addRelations = (relations, diagram_key,
                      point_idx_pair, data) => {
  const point = data.pairings[diagram_key][point_idx_pair[0]][point_idx_pair[1]]
  const relations_to_add = point[1];
  Object.entries(relations_to_add).forEach(([new_key, new_relations]) => {
    return new_relations.forEach(new_rel => {
      if (!relations[new_key].includes(new_rel)) {
  ...
  Object.entries(data.pairings).forEach(([diagram_key, diagram]) => {
    diagram.forEach((diagram_d, d) => {
      diagram_d.forEach(([point, relations], point_idx) => {
        ...
        console.log(countRelations(relations));
      });
    });
  });
  return data;
